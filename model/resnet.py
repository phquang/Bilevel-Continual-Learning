...
class CustomResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False):
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        #self.fc = nn.Linear(512 * block.expansion, num_classes)
        self.ncm = NCM_classifier(512 * block.expansion, 50)
    ...
    def update_means(self, x, y, alpha = 0):
        self.ncm.update_means(x,y, alpha = alpha)
...
class NCM_classifier(nn.Module):
    def __init__(self, features, classes= 50,  n_tasks = 10, alpha= 0.9):
        ...
        self.means = nn.Parameter(torch.randn(classes,features),requires_grad = False).cuda()
        self.running_means = nn.Parameter(torch.randn(classes,features),requires_grad = False).cuda()
    ...
    def compute_offsets(self, t):
        meta = [[0,10]] +  [[i,i+5] for i in range(10,49,5)]
        offsets = meta[t]
    ...
    def forward(self, x,t):
        ...
        classes = offset2-offset1
        means_reshaped = means.view(1, classes, self.features).expand(x.shape[0], classes, self.features)
        features_reshaped = x.view(-1,1,self.features).expand(x.shape[0], classes, self.features)
...
def ResNet18(pretrained=True, num_classes = 50, **kwargs):
    ...
    model.fc = nn.Linear(512, num_classes.item())
    return model

def NCMResNet18(pretrained=False, num_classes = 50, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = CustomResNet(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        print('Loading pretrained model')
...
'''
ncm = ncm_resnet18(pretrained = True).cuda()
x1 = torch.rand(1,3, 128, 128).cuda()
y = ncm(x1)
yy = ncm.predict(y)
'''
